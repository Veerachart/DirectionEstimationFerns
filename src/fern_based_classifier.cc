...
void fern_based_classifier::recognize_interpolate(vector<float> &fisheye_HOG_descriptor, cv::Mat& cropped_head_img, int& output_class, int& output_angle) {
	int * leaves_index = Ferns->drop(fisheye_HOG_descriptor, cropped_head_img);
	...
	if (class_index+1 >= number_of_classes) {
		high = 0;
